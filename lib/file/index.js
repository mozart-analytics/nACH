...
var batchSequenceNumber = 1; // NOTE: Changed for compatibility with BPOP nacha format. Default was 0.
...
File.prototype.generateBatches = function(done1) {
	...
	async.each(this._batches, function(batch, done2) {	
		...
		async.each(batch._entries, function(entry, done3) {
            entry.fields.traceNumber.value = (entry.fields.traceNumber.value) ?  entry.fields.traceNumber.value : self.header.immediateDestination.value.slice(0,8) + utils.pad(addendaCount, 7, false, '0'); // TODO: This should be specified as a number and then padded like in the instruction executed when the conditional is false.
